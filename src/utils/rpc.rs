...
use eore_api::{
    consts::{CONFIG_ADDRESS, TREASURY_ADDRESS, EPOCH_DURATION},
...
use solana_sdk::{clock::Clock, hash::Hash, clock::UnixTimestamp};
...
/// Retrieves the start timestamp of the current BITZ epoch.
///
/// It fetches the current cluster time (from Clock sysvar) and uses the known
/// EPOCH_DURATION constant to calculate the epoch start time.
///
/// Returns:
/// - Ok(UnixTimestamp): The Unix timestamp (seconds) of the current epoch start.
/// - Err(anyhow::Error): If fetching clock fails.
pub async fn get_current_epoch_start(client: &RpcClient) -> Result<UnixTimestamp, anyhow::Error> {
    // Fetch the current clock time
    let clock = get_clock(client).await?;
    let current_timestamp = clock.unix_timestamp;

    // Use the constant epoch duration
    let epoch_duration = EPOCH_DURATION; // Use the imported constant

    // Ensure epoch_duration is positive (though constants usually are)
    if epoch_duration <= 0 {
        // This check might be less critical for a constant but good for safety
        return Err(anyhow::anyhow!(
            "Invalid epoch duration constant: {}",
            epoch_duration
        ));
    }

    // Calculate epoch start time: floor(current_timestamp / epoch_duration) * epoch_duration
    let epoch_start_timestamp = (current_timestamp / epoch_duration as i64) * epoch_duration as i64;
